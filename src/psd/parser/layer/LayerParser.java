    ...
    private void parseMaskAndAdjustmentData(PsdInputStream stream) throws IOException {
        ...
        if (size > 0) {
            ...
            if (size == 20) {
                ...
                mask.mask = mask.new AdjustMask();
                
                mask.mask.relative = (realFlags & 0x01) != 0;
                mask.mask.disabled = ((realFlags >> 1) & 0x01) != 0;
                mask.mask.invert = ((realFlags >> 2) & 0x01) != 0;

                mask.mask.defaultColor = stream.readByte() & 0xff;
                assert mask.mask.defaultColor == 0 || mask.mask.defaultColor == 255;

                mask.mask.top = stream.readInt();
                mask.mask.left = stream.readInt();
                mask.mask.bottom = stream.readInt();
                mask.mask.right = stream.readInt();
            }
            if (handler != null) {
...
//            System.out.print("	 " + tag+","+size + ", "+ (prevPos + size - stream.getPos()));
//           
